...
export const AdminProvider = ({ children }: TAdminProviderProps) => {
    ...
    orderData?.reduce?.(
      (
        ...
        if (order.state === "active") {
          const hasUnapproved = order.products.some(
            ({ adminApproved }) => !adminApproved
          );
          const hasApproved = order.products.some(
            ({ adminApproved }) => adminApproved
          );

          // If all products are approved, move to active orders
          if (hasApproved && !hasUnapproved) {
            acc.orderActive.push(order);
          }
          // If there are unapproved products, keep in requests
  ...
  [orderRequest, orderActive, orderHistory].forEach((arr) =>
    arr?.sort?.(sortByDate)
  );
