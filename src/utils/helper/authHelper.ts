  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(cred) {
        ...
        await connectDB();
        const credential = isEmailValid(cred?.username)
          ? { email: cred?.username }
    ...
    CredentialsProvider({
      ...
      async authorize(cred) {
        ...
        if (!cred?.address) throw new Error("Pickup address is required");
        ...
        const account = await Accounts.findOne<TAccount>({
          username: cred?.restaurant,
        }).populate("profile");

        if (!account) throw new Error("Restaurant not found.");
        if (!account?.profile?.addresses?.includes(cred?.address))
