...
export default function MenuContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [itemsData, toppingsData, locationsData] = await Promise.all([
          apiClient.get(
            `/menu/items${category ? `?category=${category}` : ""}`
          ),
          apiClient.get("/menu/toppings"),
        ...
        for (const item of itemsData.items) {
          try {
            const ratingsData = await apiClient.get(
              `/reviews?menuItemId=${item._id}&approved=true`
            );
        ...
        if (locationsData.locations.length > 0) {
          setSelectedLocation(locationsData.locations[0]._id);
